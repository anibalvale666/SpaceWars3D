...
bool GameMap::is_in_player_zone(double pos_x, double pos_y)
{
	...
	if ((pos_y < max_y) && (pos_y > screen_limits_y.first) && (pos_x < screen_limits_x.second) && (pos_x > screen_limits_x.first))
		return true;
...
void Player::move()
{
	...
	if (Player1->IsConnected())
	{
		...
		if ((Player1->GetState().Gamepad.bRightTrigger / 255.0f) > 0.0)// de 0 a 1
		{
			if (reload_time == 0)
			{
				fire();
				if ((Player1->GetState().Gamepad.bRightTrigger / 255.0f) < 0.33)
				{
					reload_time = 150;
				}
				else if ((Player1->GetState().Gamepad.bRightTrigger / 255.0f) < 0.66)
				{
					reload_time = 120;
				}
				else if ((Player1->GetState().Gamepad.bRightTrigger / 255.0f) < 1)
				{
					reload_time = 90;
				}
				else if ((Player1->GetState().Gamepad.bRightTrigger / 255.0f) == 1)
			...
			if (Player1->GetState().Gamepad.bRightTrigger / 255.0f == 1)
			{
				countTrigger++;
			}
			else
